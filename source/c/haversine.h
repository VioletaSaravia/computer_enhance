...
f64 Haversine(HaversinePair p, f64 radius) {
    const f64 dLat = Deg(p.y1 - p.y0);
    const f64 dLon = Deg(p.x1 - p.x0);
    const f64 lat1 = Deg(p.y0);
    const f64 lat2 = Deg(p.y1);
...
f64 SumHaversines(HaversinePair* pairs, u64 count) {
    PROFILE_FUNCTION();
    const f64 coef = 1.0 / (f64)count;
    f64       sum  = 0.0;
...
Array<HaversinePair> ParseHaversineJson(const Array* bytes, int expected_count = 1024) {
    PROFILE_FUNCTION();
    PROFILE_ADD_BANDWIDTH(bytes.len);
